...
const checkCommentCanBeUpdated = async (id: string, user: User | null | undefined): Promise<[boolean, Post]> => {
    if (!user || !user.id) {
...
const createComment = async (data: CreateCommentInput, user: User | null | undefined): Promise<Comment> => {
    if (!user || !user.id) {
        throw new Error('User not authenticated');
    }
...
const archiveComment = async (id: string, user: User | null | undefined): Promise<Comment> => {
    ...
    return mapDBCommentToComment(await commentRepository.archiveComment(id), post);
}

const updateComment = async (id: string, data: UpdateCommentInput, user: User | null | undefined): Promise<Comment> => {
    const [canBeUpdated, post] = await checkCommentCanBeUpdated(id, user);
    ...
    return mapDBCommentToComment(await commentRepository.updateComment(id, payload), post);
}



const commentService = { findAvailableComments, createComment, updateComment, archiveComment, findCommentById };
